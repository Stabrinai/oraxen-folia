            ...
            public <T extends BlockStateHolder<T>> boolean setBlock(BlockVector3 pos, T block) throws WorldEditException {
                ...
                if (blockData.getMaterial() == Material.NOTE_BLOCK) {
                    if (mechanic != null && Settings.WORLDEDIT_NOTEBLOCKS.toBool()) {
                        Bukkit.getRegionScheduler().run(OraxenPlugin.get(), loc, task -> OraxenBlocks.place(mechanic.getItemID(), loc));
                    }
                    ...
                    Bukkit.getRegionScheduler().run(OraxenPlugin.get(), loc, task -> OraxenBlocks.remove(loc, null));
                }
